...
# set parameters for nested grid search
params = {
...
for measure in measures:
    for band in bands:
        ...
        #results['Feature_Name'] = [name[:10] for name in results['Feature_Name']]

        #  plt.bar(results['Feature_Name'][:10], results['Importance_Score'][:10])
        # plt.title('Dyad Importance for Alpha Coherence Classifier Model')
        # plt.xlabel('Dyad')
...
with open('initialregression/'+ "mse_all_measureonly.json", "w") as file:
    json.dump(mse_all, file)

with open('initialregression/' + "mae_all_measureonly.json", "w") as file:
    json.dump(mae_all, file)

with open('initialregression/'+"r2_all_measureonly.json", "w") as file:
    json.dump(r2_all, file)

predicted_values.to_csv('initialregression/'+'predicted_values_measureonly.csv')